...
variant_params = {
    "retention_clip": {
        "mount_pins": "npth",
        "datasheet": "https://www.molex.com/pdm_docs/sd/436500221_sd.pdf",
        "C_minus_B": 4.3,
        "part_code": "43650-{n:02}21",
        "alternative_codes": ["43650-{n:02}22", "43650-{n:02}23"],
    },
    "solder_tab": {
        "mount_pins": "solder",
        "datasheet": "https://www.molex.com/pdm_docs/sd/436500224_sd.pdf",
        "C_minus_B": 11.2,
        "part_code": "43650-{n:02}24",
        "alternative_codes": ["43650-{n:02}25", "43650-{n:02}26"],
    },
}
...
def generate_one_footprint(pins_per_row, variant, configuration):
    ...
    kicad_mod.append(
        Line(
            start={"x": body_edge["left"], "y": body_edge["top"] + chamfer["y"]},
            end={"x": body_edge["left"] + chamfer["x"], "y": body_edge["top"]},
            width=configuration["fab_line_width"],
            layer="F.Fab",
        )
    )
    ...
    kicad_mod.append(
        Line(
            start={"x": body_edge["right"], "y": body_edge["top"] + chamfer["y"]},
            end={"x": body_edge["right"] - chamfer["x"], "y": body_edge["top"]},
            width=configuration["fab_line_width"],
            layer="F.Fab",
        )
    )
    ...
    p1m_poly = tab_poly = [
        ...
        {"x": pad1_x, "y": body_edge["bottom"] - p1m_sl / sqrt(2)},
        {"x": pad1_x + p1m_sl / 2, "y": body_edge["bottom"]},
    ]
    kicad_mod.append(
        PolygoneLine(
